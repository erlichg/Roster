...
exports.monthNum = function(month) {
    return typeof month === "number"
        ...
        : month[charCodeAt](0) >= 1488 &&
          ...
          /('|")/.test(month)
            ? gematriya(month)
            : month[charCodeAt](0) >= 48 &&
              month[charCodeAt](0) <= 57 /* number */
                ? parseInt(month, 10)
                : monthFromName(month);
};
...
function hebElapsedDays(hYear) {
    ...
    let p_elapsed = 204 + 793 * (m_elapsed % 1080);
    ...
    let alt_day =
        day +
        (parts >= 19440 ||
            (day % 7 == 2 && parts >= 9924 && !LEAP(hYear)) ||
            (day % 7 == 1 && parts >= 16789 && LEAP(hYear - 1)));

    return (
        alt_day + (alt_day % 7 === 0 || alt_day % 7 == 3 || alt_day % 7 == 5)
    );
}
...
function filter(self, fun, thisp) {
    ...
    for (var i in t) {
        if (t.hasOwnProperty(i)) {
            const val = t[i]; // in case fun mutates it
            if (fun.call(thisp, val, i, t)) {
                // define property on res in the same manner as it was originally defined
                const props = Object.getOwnPropertyDescriptor(t, i);
                props.value = val;
