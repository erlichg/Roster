...
const Sedra = require("./sedra");

const dafyomi = require("./dafyomi");
...
const getYearObject = "getYearObject";
...
const HebcalProto = Hebcal.prototype;

const MonthProto = Month.prototype;

const GregYearProto = GregYear.prototype;

const GregMonthProto = GregMonth.prototype;

const HDateProto = HDate.prototype;
...
function Hebcal(year, month) {
    const me = this; // whenever this is done, it is for optimizations.
    if (!year) {
    ...
    if (month) {
        ...
        if (Array.isArray(month)) {
            me.months = month[map](i => {
                const m = new Month(i, year);
                defProp(m, "__year", {
                    configurable: true,
                    writable: true,
                    value: me
...
HebcalProto[getDay] = function(day) {
    ...
    const rosh =
        me[find](29, months.ELUL)[0].abs() + 1 - me[find](1, NISAN)[0].abs(); // number of days between Nisan and Tishrei
    if (day <= rosh) {
...
HebcalProto.days = function() {
    return [].concat.apply([], this.months[map](m => m.days));
};
...
HebcalProto.findParsha = function(parsha, o) {
    const langs = o ? [o] : ["s", "a", "h"]; // FIXME: abstract this away somewhere
    const days = this.filter(d =>
        Math.max.apply(null, langs.map(l => d.getSedra(l).indexOf(parsha) + 1))
    );
    ...
    if (arguments[length] === 1) {
        ...
        if (Array.isArray(day)) {
            return [].concat.apply(
                [],
                day[map](d =>
                    me[find][Array.isArray(d) ? "apply" : "call"](me, d)
                )
            );
        }
        if (day instanceof HDate) {
        ...
        if (Array.isArray(month)) {
            return [].concat.apply([], month[map](m => me[find](day, m)));
        }
        if (typeof month === "string") {
...
HebcalProto[find][strings] = function strings(str) {
    ...
    const func = strings[str.replace(/\s/g, "_").toLowerCase()];
    if (!split[length]) {
        return [];
    }
    if (func) {
...
HebcalProto[find][strings].succot = HebcalProto[find][
    strings
].succos = HebcalProto[find][strings].sukkos =
...
HebcalProto[find][strings].rosh_hashanah =
    HebcalProto[find][strings].rosh_hashana;
...
function Month(month, year) {
    ...
    me.days = c.range(1, c.daysInMonth(month, year))[map](i => {
        const d = new HDate(i, month, year);
        defProp(d, "__month", {
            configurable: true,
            writable: true,
            value: me
    ...
    me.holidays = c.filter(
        holidays.year(year),
        h => h[0].date[getMonth]() == month
    );
...
MonthProto.molad = function() {
    ...
    retMolad.doy = new HDate(
        toInt(1 + 29 * m_elapsed + h_elapsed / 24)
    ).getDay();
    retMolad.hour = Math.round(h_elapsed % 24);
    const chalakim = toInt(parts % 1080);
...
MonthProto[find] = function find_f(day) {
    ...
    if (Array.isArray(day)) {
        return [].concat.apply([], day[map](d => me[find](d)));
    }
    if (
...
MonthProto[find][strings].shabbos_mevarchim = MonthProto[find][
    strings
].shabbos_mevorchim =
...
(function() {
    const orig = {}; // slightly less overhead when using unaffiliated HDate()s
    [prev, next].forEach(func => {
        orig[func] = HDateProto[func];
        HDateProto[func] = function() {
            const day = orig[func].call(this);
            if (!this.__month) {
                return day;
            }
...
HDateProto.getSedra = (function() {
    ...
    return function(o) {
        let sedraYear = __cache[this[getFullYear]()];
        if (!sedraYear || sedraYear.il != this.il) {
            sedraYear = __cache[this[getFullYear]()] = new Sedra(
                this[getFullYear](),
                this.il
            );
        }
        return sedraYear.get(this)[map](p => c.LANG(p, o));
    };
})();
...
HDateProto.holidays = function(all) {
    ...
    const days = me[getYearObject]().holidays[me];
    return days
        ? days.filter(h => (all ? true : !h.routine() && h.is(me)))[map](h => {
...
["candleLighting", "havdalah"].forEach(prop => {
    HDateProto[prop] = function() {
        const me = this;

        let hd = me.holidays(true).filter(h => h.is(me));
        if (hd.length) {
            hd = c.filter(hd.map(h => h[prop]()), true);
        }
...
HDateProto.omer = function() {
    ...
    const greg = me.greg().getTime();

    const year = me[getFullYear]();
    if (
...
HDateProto.tachanun = (function() {
    const NONE = (tachanun.NONE = 0);

    const MINCHA = (tachanun.MINCHA = 1);

    const SHACHARIT = (tachanun.SHACHARIT = 2);

    const ALL_CONGS = (tachanun.ALL_CONGS = 4);
...
HDateProto.hallel = (function() {
    const NONE = (hallel.NONE = 0);

    const HALF = (hallel.HALF = 1);

    const WHOLE = (hallel.WHOLE = 2);
    ...
    function hallel() {
        ...
        const half =
            ...
            (__cache.half[y] = []
                .concat(
                    year[find]("Rosh Chodesh").filter(
                        rc => rc[getMonth]() != TISHREI
...
(function(events) {
    ...
    function checkTimes() {
        ...
        function close(obj, compare) {
            return c.filter(
                c[map](obj, time => time - nowGreg),
                time => time > 0 && time - compare < 0
            );
        }

        var nowGreg = new Date();

        const almostTime = close(now.getZemanim(), events.beforeZeman);

        const customTimes = close(events.customs, events.refreshInterval);

        for (const zeman in almostTime) {
            events.emit("almostZeman", zeman, almostTime[zeman]);
            if (almostTime[zeman] < events.refreshInterval) {
                events.emit("atZeman", zeman);
            }
        }
        for (const custom in customTimes) {
...
function GregYear(year, month) {
    ...
    me.hebyears = [].concat
        .apply([], me.months[map](m => m.hebmonths[map](hm => hm.year)))
        .filter(
            (val, i, arr) => arr.indexOf(val) === i // keep unique values only
        );

    me.holidays = c.filter(
        holidays.year(me.hebyears[0]),
        h =>
            h[0].date.greg()[getFullYear]() === year &&
            me.months.filter(
                m =>
                    // don't keep ones that are out of bounds
                    m.month === h[0].date.greg()[getMonth]() + 1
            )[length]
    );
    if (me.hebyears[1]) {
        extend(
...
function GregMonth(month, year) {
    ...
    me.days = c.range(1, greg.daysInMonth(month, year))[map](i => {
        const d = new HDate(new Date(year, month - 1, i));
    ...
    ].filter((val, i, arr) => i === 0 || val.month != arr[0].month);
